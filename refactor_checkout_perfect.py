...
def get_lines(start_line_1_index, end_line_1_index):
    # Returns from start_line to end_line inclusive
    return "".join(lines[start_line_1_index-1 : end_line_1_index])

shipping_block = get_lines(378, 461)
delivery_block = get_lines(466, 506)
payment_block = get_lines(511, 590)
review_block = get_lines(595, 670)

# Success Block: Inside the if(orderPlaced) return ( ... );
success_block = get_lines(284, 306) # from <PageWrapper> to </PageWrapper>

# Summary Block
summary_block = get_lines(707, 758) # from <div className="lg:col-span-2"> to </div>
...
export function CheckoutSuccess({ orderId }: { orderId: string }) {
  return (
""" + success_block.replace("if (items.length === 0 && !orderPlaced) {", "").replace("return (", "").replace(");", "").strip() + """
  );
}
"""
...
# Clean up Success Comp correctly: it's literally just the PageWrapper block, we strip extra returns.
success_block_clean = get_lines(285, 305) # Just the inside of return ()
success_comp_clean = f"""import {{ m as motion }} from 'framer-motion';
import {{ Check }} from 'lucide-react';
import Link from 'next/link';
import PageWrapper from '@/components/layout/PageWrapper';
...
"""
with open('src/app/checkout/components/CheckoutSuccess.tsx', 'w') as f: f.write(success_comp_clean)
with open('src/app/checkout/components/OrderSummary.tsx', 'w') as f: f.write(summary_comp)

# Replacements in file. We do them from BOTTOM to TOP so indices don't shift!
lines[707-1:758] = ['              <OrderSummary items={items} subtotal={subtotal} shipping={shipping} total={total} />\n']
lines[595-1:670] = ['                  <CheckoutReviewStep form={form} receiptFile={receiptFile} uploadState={uploadState} uploadProgress={uploadProgress} handleFileChange={handleFileChange} retryUpload={retryUpload} setReceiptFile={setReceiptFile} setUploadState={setUploadState} currentStep={currentStep} />\n']
lines[511-1:590] = ['                  <CheckoutPaymentStep form={form} updateForm={updateForm} currentStep={currentStep} />\n']
lines[466-1:506] = ['                  <CheckoutDeliveryStep form={form} updateForm={updateForm} subtotal={subtotal} FREE_DELIVERY_THRESHOLD={FREE_DELIVERY_THRESHOLD} currentStep={currentStep} />\n']
lines[378-1:461] = ['                  <CheckoutShippingStep form={form} updateForm={updateForm} errors={errors} savedAddresses={savedAddresses} selectedAddressId={selectedAddressId} handleSelectAddress={handleSelectAddress} getInputClass={getInputClass} currentStep={currentStep} />\n']
lines[284-1:306] = ['      return <CheckoutSuccess orderId={orderId} />;\n']
