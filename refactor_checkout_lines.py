...
shipping_block = get_lines(376, 461)
delivery_block = get_lines(463, 506)
payment_block = get_lines(508, 590)
review_block = get_lines(592, 670)
success_block = get_lines(284, 306) # inside if
summary_block = get_lines(707, 758)
...
lines[375:461] = ['                <CheckoutShippingStep form={form} updateForm={updateForm} errors={errors} savedAddresses={savedAddresses} selectedAddressId={selectedAddressId} handleSelectAddress={handleSelectAddress} getInputClass={getInputClass} currentStep={currentStep} />\n']
# Note: we need to adjust indices because we are deleting lines.
# Instead of deleting from the original lines array, we should replace from bottom to top.
# bottom to top:
lines[706:758] = ['              <OrderSummary items={items} subtotal={subtotal} shipping={shipping} total={total} />\n']
lines[591:670] = ['                <CheckoutReviewStep form={form} receiptFile={receiptFile} uploadState={uploadState} uploadProgress={uploadProgress} handleFileChange={handleFileChange} retryUpload={retryUpload} setReceiptFile={setReceiptFile} setUploadState={setUploadState} currentStep={currentStep} />\n']
lines[507:590] = ['                <CheckoutPaymentStep form={form} updateForm={updateForm} currentStep={currentStep} />\n']
lines[462:506] = ['                <CheckoutDeliveryStep form={form} updateForm={updateForm} subtotal={subtotal} FREE_DELIVERY_THRESHOLD={FREE_DELIVERY_THRESHOLD} currentStep={currentStep} />\n']
lines[375:461] = ['                <CheckoutShippingStep form={form} updateForm={updateForm} errors={errors} savedAddresses={savedAddresses} selectedAddressId={selectedAddressId} handleSelectAddress={handleSelectAddress} getInputClass={getInputClass} currentStep={currentStep} />\n']
lines[283:307] = ['  if (orderPlaced) return <CheckoutSuccess orderId={orderId} />;\n']
