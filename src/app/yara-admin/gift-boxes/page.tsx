  ...
  const fetchData = () => {
    ...
    Promise.all([
      ...
      const mapped: RawBox[] = rawBoxes.map((r) => ({
        ...
        images: (r.images || []).map((fn: string) => buildImageUrl(r.collectionId, r.id, fn)),
        imageFiles: r.images || [],
      ...
      setBoxes((prev) =>
        prev.map((b) =>
          b.id === selectedBox.id
            ? {
                ...b,
                images: (updated.images || []).map((fn: string) =>
                  buildImageUrl(updated.collectionId, updated.id, fn)
                ),
                imageFiles: updated.images || [],
                category: Array.isArray(updated.category) ? updated.category[0] || '' : updated.category || '',
                fixed_items: updated.fixed_items || [],
  ...
  return (
    <div className="space-y-6 max-w-7xl">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
        ...
        <button 
          onClick={fetchData}
          className="px-4 py-2 text-burgundy/60 hover:text-burgundy hover:bg-burgundy/5 rounded-full transition-colors self-start sm:self-auto flex items-center gap-2 text-sm font-medium disabled:opacity-50 bg-white shadow-sm border border-burgundy/10"
          title="Refresh Data"
          disabled={loading}
        >
